...
import MSDogeSig from "../contracts/MSDogeSig.json";
...
const Lawis = () => {
    ...
    useEffect(async() => {
        ...
        if (_web3) {
            ...
            const _MsDogeSig = new _web3.eth.Contract(MSDogeSig, SigAddress);
            setWEB3(_web3);
            setMsdoge(_MSDoge);
    ...
    useEffect(async() => {
        if (msdogeSig && isConnected && ownerAddress != 'Loading...') {
        console.log('changed');
            await initalSetting();
    ...
    }, [msdogeSig, isConnected, ownerAddress])
    ...
    const initalSetting = async () => {
        const _ownerBalance = await msdoge.methods.balanceOf(ownerAddress).call();
        setOwnerBalance(_ownerBalance / (Math.pow(10, 9)));
        const _totalSupply = await msdoge.methods.totalSupply().call();
        setTotalSupply(_totalSupply / (Math.pow(10, 9)));
        const _sentAmount = await msdogeSig.methods.getTransferedAmount().call({ from: ownerAddress });
    ...
    const createTransferRequest = async () => {
        ...
        setIsLoading(true);
        try{
            const existed = await msdoge.methods.allowance(ownerAddress, SigAddress);
            const newAllow = Number(existed) + Number(web3.utils.toWei(transferAmount.toString(), "gwei"));
            await msdoge.methods.approve(SigAddress, newAllow).
            send({ from : ownerAddress })
            .on('receipt', async(receipt) => {
                await msdogeSig.methods.newTransferRequest(transferAddress, web3.utils.toWei(transferAmount.toString(), "gwei"))
                .send({ from: ownerAddress })
                .on('receipt', async(res) => {
                    NotificationManager.info("Added successfully!", "Info");
                    const len = await msdogeSig.methods.getRequestLength().call();
                    const item = await msdogeSig.methods.getTransferItem(len - 1).call();
    ...
    const approveTransferRequest = async () => {
        ...
        try {
            setIsLoading(true);
            await msdogeSig.methods.approveTransferRequest(endTransferRequest.index)
            .send({ from: ownerAddress })
            .on('receipt', async(res) => {
    ...
    const declineTransferRequest = async (idx) => {
        ...
        try {
            setIsLoading(true);
            await msdogeSig.methods.declineTransferRequest(endTransferRequest.index)
            .send({ from: ownerAddress })
            .on('receipt', async(res) => {
                NotificationManager.success("Sent successfully!", "Success");
                await getRequestedList();
    ...
    const getRequestedList = async () => {
        ...
        let list = await msdogeSig.methods.getRequestList().call({ from: ownerAddress });
    ...
    const getAirDropList = async() => {
        ...
        let list = await msdogeSig.methods.getAirDropList().call({ from : ownerAddress });
    ...
    const approveRequestList = async () => {
        ...
        try {
            setIsLoading(true);
            await msdogeSig.methods.approveTransferListRequest(sendMultiple)
            .send({ from: ownerAddress })
            .on('receipt', async(res) => {
                NotificationManager.success("Sent successfully!", "Success");
                await getRequestedList();
    ...
    const declineRequestList = async () => {
        ...
        try {
            setIsLoading(true);
            await msdogeSig.methods.declineTransferListRequest(sendMultiple)
            .send({ from: ownerAddress })
            .on('receipt', async(res) => {
    ...
    const createBurnRequest = async() => {
        ...
        const requestItem = await msdogeSig.methods.getBurnRequest().call();
        ...
        try {
            await msdoge.methods.approve(SigAddress, web3.utils.toWei(burnAmount.toString(), "gwei")).
            send({ from : ownerAddress })
            .on('receipt', async(receipt) => {
                await msdogeSig.methods.newBurnRequest(web3.utils.toWei(burnAmount.toString(),"gwei"))
                .send({ from: ownerAddress })
                .on('receipt', (res) => {
    ...
    const approveBurnRequest = async() => {
        ...
        try {
            await msdogeSig.methods.approveBurnRequest()
            .send({ from: ownerAddress })
            .on('receipt', receipt => {
    ...
    const declineBurnRequest = async() => {
        ...
        try {
            ...
            await msdogeSig.methods.declineBurnRequest()
            .send({ from: ownerAddress })
            .on('receipt', (res) => {
                NotificationManager.success("Declined successfully", "Success");
                setReqeustBurn(false);
    ...
    const AirDrop = async() => {
        ...
        try {
            for (let i = 0; i < airDropList.length; i ++) {
                if(chunkList.filter((c) => c[0] === airDropList[i].addresses ).length > 0) continue;
                totalBalance += Number(airDropList[i].balances);
                chunkList.push([
                    airDropList[i].addresses,
                    web3.utils.toWei(airDropList[i].balances, 'gwei')
                ])
            }

            const ownerBalance = await msdoge.methods.balanceOf(ownerAddress).call();
            ...
            .send({ from: ownerAddress })
            .on('receipt', async(res) => {                
                await msdogeSig.methods.airDrop(chunkList)
                .send({ from : ownerAddress })
                .on('receipt', res => {
    ...
    const getLatestItem = async() => {
        const transfer = await msdogeSig.methods.getLatestTransferRequest().call({ from: ownerAddress });
        if (transfer.item.isActive) {
            setEndTransferRequest(transfer.item);
            setDecTransferNumber(transfer.cancel);
        }

        const burn = await msdogeSig.methods.getBurnRequest().call();
